...
import com.forge.fex.verilogprime.gen.VerilogPrimeParser.Generate_block_part3Context;
...
public class Generate_block_part3ContextExt extends AbstractBaseExt {
	...
	@Override
	public Generate_block_part3Context getContext() {
		return (Generate_block_part3Context) contexts.get(contexts.size() - 1);
	}
		...
		if (ctx != null) {
			if (ctx instanceof Generate_block_part3Context) {
				...
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "
						+ Generate_block_part3Context.class.getName());
			}
		} else {
			addToContexts(null);
		}
