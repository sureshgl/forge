package com.forge.fex.verilogprime.ext;

import org.antlr.v4.runtime.ParserRuleContext;

import com.forge.fex.verilogprime.gen.VerilogPrimeParser.Class_qualifierContext;
import com.forge.fex.verilogprime.utils.PopulateExtendedContextVisitor;

public class Class_qualifierContextExt extends AbstractBaseExt {

	public Class_qualifierContextExt(Class_qualifierContext ctx) {
		addToContexts(ctx);
		parent = ctx;
	}

	@Override
	public Class_qualifierContext getContext() {
		return (Class_qualifierContext) contexts.get(contexts.size() - 1);
	}

	@Override
	public ParserRuleContext getContext(String str) {
		return new PopulateExtendedContextVisitor().visit(getPrimeParser(str).class_qualifier());
	}

	@Override
	public void setContext(ParserRuleContext ctx) {
		if (ctx != null) {
			if (ctx instanceof Class_qualifierContext) {
				addToContexts(ctx);
			} else {
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "
						+ Class_qualifierContext.class.getName());
			}
		} else {
			addToContexts(null);
		}
	}
}