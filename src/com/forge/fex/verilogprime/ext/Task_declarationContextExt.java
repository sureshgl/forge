...
import com.forge.fex.verilogprime.gen.VerilogPrimeParser.Task_declarationContext;
...
public class Task_declarationContextExt extends AbstractBaseExt {
	...
	@Override
	public Task_declarationContext getContext() {
		return (Task_declarationContext) contexts.get(contexts.size() - 1);
	}
		...
		if (ctx != null) {
			if (ctx instanceof Task_declarationContext) {
				...
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "
						+ Task_declarationContext.class.getName());
			}
		} else {
			addToContexts(null);
		}
