...
import com.forge.fex.verilogprime.gen.VerilogPrimeParser.T0x_path_delay_expressionContext;
...
public class T0x_path_delay_expressionContextExt extends AbstractBaseExt {
	...
	@Override
	public T0x_path_delay_expressionContext getContext() {
		return (T0x_path_delay_expressionContext) contexts.get(contexts.size() - 1);
	}
		...
		if (ctx != null) {
			if (ctx instanceof T0x_path_delay_expressionContext) {
				...
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "
						+ T0x_path_delay_expressionContext.class.getName());
			}
		} else {
			addToContexts(null);
		}
