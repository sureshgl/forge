package com.forge.fex.verilogprime.ext;

import org.antlr.v4.runtime.ParserRuleContext;

import com.forge.fex.verilogprime.gen.VerilogPrimeParser.EndclockingstrContext;
import com.forge.fex.verilogprime.utils.PopulateExtendedContextVisitor;

public class EndclockingstrContextExt extends AbstractBaseExt {

	public EndclockingstrContextExt(EndclockingstrContext ctx) {
		addToContexts(ctx);
		parent = ctx;
	}

	@Override
	public EndclockingstrContext getContext() {
		return (EndclockingstrContext) contexts.get(contexts.size() - 1);
	}

	@Override
	public ParserRuleContext getContext(String str) {
		return new PopulateExtendedContextVisitor().visit(getPrimeParser(str).endclockingstr());
	}

	@Override
	public void setContext(ParserRuleContext ctx) {
		if (ctx != null) {
			if (ctx instanceof EndclockingstrContext) {
				addToContexts(ctx);
			} else {
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "
						+ EndclockingstrContext.class.getName());
			}
		} else {
			addToContexts(null);
		}
	}
}