package com.forge.fex.verilogprime.ext;

import org.antlr.v4.runtime.ParserRuleContext;

import com.forge.fex.verilogprime.gen.VerilogPrimeParser.Library_identifierContext;
import com.forge.fex.verilogprime.utils.PopulateExtendedContextVisitor;

public class Library_identifierContextExt extends AbstractBaseExt {

	public Library_identifierContextExt(Library_identifierContext ctx) {
		addToContexts(ctx);
		parent = ctx;
	}

	@Override
	public Library_identifierContext getContext() {
		return (Library_identifierContext) contexts.get(contexts.size() - 1);
	}

	@Override
	public ParserRuleContext getContext(String str) {
		return new PopulateExtendedContextVisitor().visit(getPrimeParser(str).library_identifier());
	}

	@Override
	public void setContext(ParserRuleContext ctx) {
		if (ctx != null) {
			if (ctx instanceof Library_identifierContext) {
				addToContexts(ctx);
			} else {
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "
						+ Library_identifierContext.class.getName());
			}
		} else {
			addToContexts(null);
		}
	}
}