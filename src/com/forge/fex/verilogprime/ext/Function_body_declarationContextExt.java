package com.forge.fex.verilogprime.ext;

import org.antlr.v4.runtime.ParserRuleContext;

import com.forge.fex.verilogprime.gen.VerilogPrimeParser.Function_body_declarationContext;
import com.forge.fex.verilogprime.utils.PopulateExtendedContextVisitor;

public class Function_body_declarationContextExt extends AbstractBaseExt {

	public Function_body_declarationContextExt(Function_body_declarationContext ctx) {
		addToContexts(ctx);
		parent = ctx;
	}

	@Override
	public Function_body_declarationContext getContext() {
		return (Function_body_declarationContext) contexts.get(contexts.size() - 1);
	}

	@Override
	public ParserRuleContext getContext(String str) {
		return new PopulateExtendedContextVisitor().visit(getPrimeParser(str).function_body_declaration());
	}

	@Override
	public void setContext(ParserRuleContext ctx) {
		if (ctx != null) {
			if (ctx instanceof Function_body_declarationContext) {
				addToContexts(ctx);
			} else {
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "
						+ Function_body_declarationContext.class.getName());
			}
		} else {
			addToContexts(null);
		}
	}
}