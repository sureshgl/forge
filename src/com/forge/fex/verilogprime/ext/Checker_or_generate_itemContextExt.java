...
import com.forge.fex.verilogprime.gen.VerilogPrimeParser.Checker_or_generate_itemContext;
...
public class Checker_or_generate_itemContextExt extends AbstractBaseExt {
	...
	@Override
	public Checker_or_generate_itemContext getContext() {
		return (Checker_or_generate_itemContext) contexts.get(contexts.size() - 1);
	}
		...
		if (ctx != null) {
			if (ctx instanceof Checker_or_generate_itemContext) {
				...
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "
						+ Checker_or_generate_itemContext.class.getName());
			}
		} else {
			addToContexts(null);
		}
