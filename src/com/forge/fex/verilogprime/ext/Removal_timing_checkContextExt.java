...
import com.forge.fex.verilogprime.gen.VerilogPrimeParser.Removal_timing_checkContext;
...
public class Removal_timing_checkContextExt extends AbstractBaseExt {
	...
	@Override
	public Removal_timing_checkContext getContext() {
		return (Removal_timing_checkContext) contexts.get(contexts.size() - 1);
	}
		...
		if (ctx != null) {
			if (ctx instanceof Removal_timing_checkContext) {
				...
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "
						+ Removal_timing_checkContext.class.getName());
			}
		} else {
			addToContexts(null);
		}
