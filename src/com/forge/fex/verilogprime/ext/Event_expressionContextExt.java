package com.forge.fex.verilogprime.ext;

import org.antlr.v4.runtime.ParserRuleContext;

import com.forge.fex.verilogprime.gen.VerilogPrimeParser.Event_expressionContext;
import com.forge.fex.verilogprime.utils.PopulateExtendedContextVisitor;

public class Event_expressionContextExt extends AbstractBaseExt {

	public Event_expressionContextExt(Event_expressionContext ctx) {
		addToContexts(ctx);
		parent = ctx;
	}

	@Override
	public Event_expressionContext getContext() {
		return (Event_expressionContext) contexts.get(contexts.size() - 1);
	}

	@Override
	public ParserRuleContext getContext(String str) {
		return new PopulateExtendedContextVisitor().visit(getPrimeParser(str).event_expression());
	}

	@Override
	public void setContext(ParserRuleContext ctx) {
		if (ctx != null) {
			if (ctx instanceof Event_expressionContext) {
				addToContexts(ctx);
			} else {
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "
						+ Event_expressionContext.class.getName());
			}
		} else {
			addToContexts(null);
		}
	}
}