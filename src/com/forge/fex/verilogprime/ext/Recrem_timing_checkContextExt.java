...
import com.forge.fex.verilogprime.gen.VerilogPrimeParser.Recrem_timing_checkContext;
...
public class Recrem_timing_checkContextExt extends AbstractBaseExt {
	...
	@Override
	public Recrem_timing_checkContext getContext() {
		return (Recrem_timing_checkContext) contexts.get(contexts.size() - 1);
	}
		...
		if (ctx != null) {
			if (ctx instanceof Recrem_timing_checkContext) {
				...
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "
						+ Recrem_timing_checkContext.class.getName());
			}
		} else {
			addToContexts(null);
		}
