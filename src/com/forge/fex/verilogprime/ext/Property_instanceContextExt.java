...
import com.forge.fex.verilogprime.gen.VerilogPrimeParser.Property_instanceContext;
...
public class Property_instanceContextExt extends AbstractBaseExt {
	...
	@Override
	public Property_instanceContext getContext() {
		return (Property_instanceContext) contexts.get(contexts.size() - 1);
	}
		...
		if (ctx != null) {
			if (ctx instanceof Property_instanceContext) {
				...
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "
						+ Property_instanceContext.class.getName());
			}
		} else {
			addToContexts(null);
		}
