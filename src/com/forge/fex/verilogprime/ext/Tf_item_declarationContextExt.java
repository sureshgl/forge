...
import com.forge.fex.verilogprime.gen.VerilogPrimeParser.Tf_item_declarationContext;
...
public class Tf_item_declarationContextExt extends AbstractBaseExt {
	...
	@Override
	public Tf_item_declarationContext getContext() {
		return (Tf_item_declarationContext) contexts.get(contexts.size() - 1);
	}
		...
		if (ctx != null) {
			if (ctx instanceof Tf_item_declarationContext) {
				...
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "
						+ Tf_item_declarationContext.class.getName());
			}
		} else {
			addToContexts(null);
		}
