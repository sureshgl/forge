...
import com.forge.fex.verilogprime.gen.VerilogPrimeParser.List_of_path_inputsContext;
...
public class List_of_path_inputsContextExt extends AbstractBaseExt {
	...
	@Override
	public List_of_path_inputsContext getContext() {
		return (List_of_path_inputsContext) contexts.get(contexts.size() - 1);
	}
		...
		if (ctx != null) {
			if (ctx instanceof List_of_path_inputsContext) {
				...
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "
						+ List_of_path_inputsContext.class.getName());
			}
		} else {
			addToContexts(null);
		}
