...
import com.forge.fex.verilogprime.gen.VerilogPrimeParser.Seq_block_part1Context;
...
public class Seq_block_part1ContextExt extends AbstractBaseExt {
	...
	@Override
	public Seq_block_part1Context getContext() {
		return (Seq_block_part1Context) contexts.get(contexts.size() - 1);
	}
		...
		if (ctx != null) {
			if (ctx instanceof Seq_block_part1Context) {
				...
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "
						+ Seq_block_part1Context.class.getName());
			}
		} else {
			addToContexts(null);
		}
