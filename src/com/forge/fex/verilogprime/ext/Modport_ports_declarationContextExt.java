...
import com.forge.fex.verilogprime.gen.VerilogPrimeParser.Modport_ports_declarationContext;
...
public class Modport_ports_declarationContextExt extends AbstractBaseExt {
	...
	@Override
	public Modport_ports_declarationContext getContext() {
		return (Modport_ports_declarationContext) contexts.get(contexts.size() - 1);
	}
		...
		if (ctx != null) {
			if (ctx instanceof Modport_ports_declarationContext) {
				...
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "
						+ Modport_ports_declarationContext.class.getName());
			}
		} else {
			addToContexts(null);
		}
