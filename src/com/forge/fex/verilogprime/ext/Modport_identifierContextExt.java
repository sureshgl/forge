...
import com.forge.fex.verilogprime.gen.VerilogPrimeParser.Modport_identifierContext;
...
public class Modport_identifierContextExt extends AbstractBaseExt {
	...
	@Override
	public Modport_identifierContext getContext() {
		return (Modport_identifierContext) contexts.get(contexts.size() - 1);
	}
		...
		if (ctx != null) {
			if (ctx instanceof Modport_identifierContext) {
				...
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "
						+ Modport_identifierContext.class.getName());
			}
		} else {
			addToContexts(null);
		}
