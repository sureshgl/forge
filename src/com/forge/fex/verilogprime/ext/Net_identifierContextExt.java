...
import com.forge.fex.verilogprime.gen.VerilogPrimeParser.Net_identifierContext;
...
public class Net_identifierContextExt extends AbstractBaseExt {
	...
	@Override
	public Net_identifierContext getContext() {
		return (Net_identifierContext) contexts.get(contexts.size() - 1);
	}
		...
		if (ctx != null) {
			if (ctx instanceof Net_identifierContext) {
				...
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "
						+ Net_identifierContext.class.getName());
			}
		} else {
			addToContexts(null);
		}
