...
foreach my $type (split(',',$opt->{'type'})) {
  ...
  if ($opt->{'sp'}) {
    ...
    $cmd = "$verilogparser -egf -o $sp_of -sp $opt->{'sp'} -sep $cut_core_name -t $cut_core_name -ets -f $df";
  } else {
    $cmd = "$verilogparser -egf -o $cut_core_unrolled -t $cut_core_name -f $df";
  }
  ...
  if ($frun->{'sva'} && $opt->{'sva'}) { # sva file
    ...
    my $df = join(" -f ", @{$filelist});
    $cmd = "$verilogparser -egf -o $cut_sva_unrolled -t $cut_sva_name -f $df 2>&1 > vp.sva.log";
    vsystem($cmd);
