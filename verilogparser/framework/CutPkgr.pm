...
my @sims       = qw (real_mem func_mem beh_mem);
...
sub parseWebJSON {
  ...
  $cfg->{'binaryPath'} = realpath($forgePath);
  ...
  $fullCmdLine .= $cfg->{'json'}->{'cmdLine'}." -fdmem -fdout -noob -rgr -fr";
  $fullCmdLine =~ s/ -nortl//;
...
sub parseMemogenJSON {
  ...
  if ($opt->{'binpath'} ne "") {
    ...
    $cfg->{'binaryPath'} = realpath($opt->{'binpath'});
  } else {
    ERROR "*********************************************************************************************";
  ...
  if ($cmdLineArgs =~ /\s+-acfg\s+\S+\s+-/) {
    $cmdLineArgs =~ s/-acfg\s+\{(.*)\}\s+-/-acfg \'{$1}\' -/;
  }elsif ($cmdLineArgs =~ /\s+-acfg\s+\S+\s*$/) {
    $cmdLineArgs =~ s/-acfg\s+\{(.*)\}\s+-/-acfg \'{$1}\'/;
    $cmdLineArgs =~ s/-acfg \s+(\S+)-/-acfg \'$1\'/;
  }
...
sub parseCmdFile {
  ...
  $cfg->{'groupName'} = $cfg->{'vendor'}."_".$cfg->{'tech'};
  $cfg->{'binaryPath'} = realpath($binPath);
  $cfg->{'binaryPath'} =~ /\/(\d+)\.(\d+)\.(\S+)/;
  $cfg->{'version'} = "$1.$2.$3";
  $cfg->{'version'} =~ s/\./_/g;
...
sub updateIbmMemFiles {
  ...
  INFO ('Appending IBM memory files to IBM*.f');

  if ($cfg->{'vendor'} ne "ibm" || $cfg->{'tech'} ne "cu32hp") {
    return 0;
  }

  my $path = $self->getOrderPath($order);
  my $docPath = "$path/doc";
  my @list;
  my @files_to_append;
  my @io_file_list;
  my @files_to_add;
  my @abs_v_files_to_append;
  File::Find::Rule->file()
  ->mindepth(1)
  ->maxdepth(1)
  ->exec(sub {my ($shortname, $docPath, $fullname) = @_;
      push @list,$shortname;
    }
  )
  ->in($docPath);
  foreach my $file (@list){
    next if(!($file =~ m/inst/));
    open(my $fh,"<","$path/doc/$file") or die("$!");
    while(<$fh>){
      chomp $_;
      $_ =~ s/\_TIE//;
      $_ =~ s/\_AFLAT//; 
      vchdir("/auto/libraries/ibm/ibm_cu32hp/memories/");
      my $pwd = `pwd`;
      chomp $pwd;
      my @verilog_files = `find * -name "$_*"`;
      if(!($verilog_files[0])){
        LOGDIE("Could not find $_ in the libraries\n");
      }
      else{
        foreach my $value(@verilog_files){
          chomp $value;
          my $string = $pwd."/".$value;
          push(@files_to_add,$string);
        }
      }
    }
  }
my @files_to_add_unq = uniq(@files_to_add);
foreach my $verilog_file(@files_to_add_unq){
  open(my $vfile,"<",$verilog_file) or die("$!");
  while(<$vfile>){
    chomp $_;
    next if($_ !~ m/FBIO/ and $_ !~ m/SBIO/);
    $_ =~ s/ .*//g;
    push(@io_file_list,$_) if($_ =~ m/FBIO/);
    push(@io_file_list,$_) if($_ =~ m/SBIO/);

  }
}
  my @io_file_list_unq = uniq(@io_file_list);
  vchdir("/auto/libraries/ibm/ibm_cu32hp/memories/memory_support_9t_v007/verilog/rtl");
  foreach my $io_file_file(@io_file_list_unq){
    my $pwd = `pwd`;
    chomp $pwd;
    my $result = `find * -name "$io_file_file*"`;
    if(!$result){
      LOGDIE "Could not find $io_file_file in the libraries";
    }
    else {
      chomp $result;
      my $abs_file = $pwd."/".$result;
      push(@files_to_append,"-v ".$abs_file);
    }
  }

  open(my $wf,">>","$path/run_sample/CU32HP_LIBS.f");
  foreach my $v_file(@files_to_append){
    print $wf "$v_file\n";
  }
  close $wf;
  return 1;
...
sub cutPackager {
  ...
  foreach my $order (0..$numOrders) {
    ...
    foreach my $sim (@sims) {
      ...
      my $srcCutV = "$orderPath/rtl_unrolled/${cutTopName}.v";
      ...
      #  $cmd = "\\cp $orderPath/rtl_enc/${cutCoreName}_${tool}.v $rtlDir";
        $cmd = "\\cp $orderPath/rtl_unrolled/${cutCoreName}.v $rtlDir";
        vsystem($cmd, $opt->{'just-print'});
        my $cutCoreEncV = "$rtlDir/${cutCoreName}.v";
        if ( !-e $cutCoreEncV) {
          LOGDIE "cutPackager: ${cutCoreName}.v file couldn't be copied to delivery directory";
          #LOGDIE "cutPackager: Encrypted ${cutCoreName}_<ncv,vcs,vsim>.v files couldn't be copied to delivery directory";
        }
      #}
...
sub postPkgSim {
  ...
  foreach my $order (0..$numOrders) {
    ...
    foreach $sim (@sims) {
      ...
      foreach my $tool (@tools) {
        if ($sim eq "func_mem" || $sim eq "beh_mem") {
          $script = "./sim.$tool.sh 2>&1 > /dev/null";
        } else {
          $script = "./sim.$tool.sh +define+MEMOIR_DBG_TEST +define+MEMOIR_COVERAGE 2>&1 > /dev/null";
        }
        $cmd = "$script 2>&1 > /dev/null";
...
sub postVCSLint {
  ...
  foreach my $order (0..$numOrders) {
    ...
    $line = "-v ../../../../rtl/${cutName}_core_donut_$cfg->{'version'}.v\n";
    print $new $line;
    $line = "-v ../../../../rtl/${cutName}_core_$cfg->{'version'}_func.v\n";
    print $new $line;
...
sub postDCLint {
  ...
  foreach my $order (0..$numOrders) {
    ...
    while( <$in> ) {
      $_ =~ s/rgr_dc_syn_script/rgr_dc_lint_script/;
      $_ =~ s/core_$cfg->{'version'}\.v.*$/core_$cfg->{'version'}\.v"/;
      print $out $_;
    }
    close $out;
    vchmod (0755, $new, $opt->{'just-print'});
